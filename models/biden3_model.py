...
""" 
BIDeN model for Task I, Mixed image decomposition across multiple domains. Max number of domain = 3.
...
class BIDEN3Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--prob', type=float, default=0.8, help='probability of adding a component')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['fake_A', 'fake_B', 'fake_C', 'real_A', 'real_B', 'real_C', 'real_input']
    ...
    def forward(self):
        ...
        self.real_input = (self.real_A * self.label[0] + self.real_B * self.label[1]
                           + self.real_C * self.label[2]) / label_sum
    ...
    def forward_test(self):
        ...
        if self.opt.test_choice == 1:
            ...
            self.real_input = (self.real_A * gt_label[0] + self.real_B * gt_label[1]
                               + self.real_C * gt_label[2]) / sum(gt_label)
            self.predict_label = self.netD(1, self.real_input).view(self.opt.max_domain)
