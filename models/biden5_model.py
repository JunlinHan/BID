...
""" 
BIDeN model for Task I, Mixed image decomposition across multiple domains. Max number of domain = 5.
...
Train:
python train.py --dataroot ./datasets/image_decom --name biden5 --model biden5 --dataset_mode unaligned5

For test:
Test a single case:
python test.py --dataroot ./datasets/image_decom --name biden5 --model biden5 --dataset_mode unaligned5 --test_input A
python test.py --dataroot ./datasets/image_decom --name biden5 --model biden5 --dataset_mode unaligned5 --test_input AB
... ane other cases.
change test_input to the case you want.

Test all cases:
python test2.py --dataroot ./datasets/image_decom --name biden5 --model biden5 --dataset_mode unaligned5
"""

class BIDEN5Model(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--max_domain', type=int, default=5, help='max number of source components.')
        parser.add_argument('--prob', type=float, default=0.6, help='probability of adding a component')
    ...
    def forward(self):
        ...
        self.real_input = (self.real_A * self.label[0] + self.real_B * self.label[1] + self.real_C * self.label[2] +
                           self.real_D * self.label[3] + self.real_E * self.label[4]) / label_sum
    ...
    def forward_test(self):
        ...
        if self.opt.test_choice == 1:
            ...
            self.real_input = (self.real_A * gt_label[0] + self.real_B * gt_label[1]
                               ...
                               + self.real_E * gt_label[4]) / sum(gt_label)
            self.fake_all = self.netE(self.real_input)
            self.predict_label = self.netD(1, self.real_input).view(self.opt.max_domain)
