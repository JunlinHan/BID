...
""" 
BIDeN model for Task I, Mixed image decomposition across multiple domains. Max number of domain = 4.
...
Train:
python train.py --dataroot ./datasets/image_decom --name biden4 --model biden4 --dataset_mode unaligned4

For test:
Test a single case:
python test.py --dataroot ./datasets/image_decom --name biden4 --model biden4 --dataset_mode unaligned4 --test_input A
python test.py --dataroot ./datasets/image_decom --name biden4 --model biden4 --dataset_mode unaligned4 --test_input AB
... ane other cases.
change test_input to the case you want.

Test all cases:
python test2.py --dataroot ./datasets/image_decom --name biden4 --model biden4 --dataset_mode unaligned4
"""

class BIDEN4Model(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--prob', type=float, default=0.7, help='probability of adding a component')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['fake_A', 'fake_B', 'fake_C', 'fake_D', 'real_A', 'real_B', 'real_C', 'real_D', 'real_input']
        self.model_names = ['D', 'E', 'H1', 'H2', 'H3', 'H4']
    ...
    def forward(self):
        ...
        self.real_input = (self.real_A * self.label[0] + self.real_B * self.label[1]
                           + self.real_C * self.label[2] + self.real_D * self.label[3]) / label_sum
    ...
    def forward_test(self):
        ...
        if self.opt.test_choice == 1:
            ...
            self.real_input = (self.real_A * gt_label[0] + self.real_B * gt_label[1]
                               + self.real_C * gt_label[2] + self.real_D * gt_label[3]) / sum(gt_label)
            self.fake_all = self.netE(self.real_input)
            self.predict_label = self.netD(1, self.real_input).view(self.opt.max_domain)
