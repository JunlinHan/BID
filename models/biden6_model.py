...
""" 
BIDeN model for Task I, Mixed image decomposition across multiple domains. Max number of domain = 6.
...
Train:
python train.py --dataroot ./datasets/image_decom --name biden6 --model biden6 --dataset_mode unaligned8

For test:
Test a single case:
python test.py --dataroot ./datasets/image_decom --name biden6 --model biden6 --dataset_mode unaligned8 --test_input A
python test.py --dataroot ./datasets/image_decom --name biden6 --model biden6 --dataset_mode unaligned8 --test_input AB
... ane other cases.
change test_input to the case you want.

Test all cases:
python test2.py --dataroot ./datasets/image_decom --name biden6 --model biden6 --dataset_mode unaligned8
"""

class BIDEN6Model(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--max_domain', type=int, default=6, help='max number of source components.')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['fake_A', 'fake_B', 'fake_C', 'fake_D', 'fake_E', 'fake_F', 'real_A', 'real_B',
                             'real_C', 'real_D', 'real_E', 'real_F', 'real_input']
        self.model_names = ['D', 'E', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6']
    ...
    def forward(self):
        ...
        self.real_input = (self.real_A * self.label[0] + self.real_B * self.label[1] + self.real_C * self.label[2] +
                           self.real_D * self.label[3] + self.real_E * self.label[4] + self.real_F * self.label[5]) / label_sum
    ...
    def compute_G_loss(self):
        ...
        self.loss_VGG = self.criterionVGG(self.fake_A, self.real_A) * self.label[0] \
                        ...
                        + self.criterionVGG(self.fake_E, self.real_E) * self.label[4] + self.criterionVGG(self.fake_F, self.real_F) * self.label[5]
    ...
    def forward_test(self):
        ...
        if self.opt.test_choice == 1:
            ...
            self.real_input = (self.real_A * gt_label[0] + self.real_B * gt_label[1]
                               ...
                               + self.real_E * gt_label[4] + self.real_F * gt_label[5]) / sum(gt_label)
            self.fake_all = self.netE(self.real_input)
            self.predict_label = self.netD(1, self.real_input).view(self.opt.max_domain)
