...
""" 
BIDeN model for Task I, Mixed image decomposition across multiple domains. Max number of domain = 8.
...
For test:
Test a single case:
python test.py --dataroot ./datasets/image_decom --name biden8 --model biden8 --dataset_mode unaligned8 --test_input A
python test.py --dataroot ./datasets/image_decom --name biden8 --model biden8 --dataset_mode unaligned8 --test_input AB
... ane other cases.
change test_input to the case you want.

Test all cases:
python test2.py --dataroot ./datasets/image_decom --name biden8 --model biden8 --dataset_mode unaligned8
"""

class BIDEN8Model(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--max_domain', type=int, default=8, help='max number of source components.')
    ...
    def __init__(self, opt):
        ...
        self.visual_names = ['fake_A', 'fake_B', 'fake_C', 'fake_D', 'fake_E', 'fake_F', 'fake_G', 'fake_H', 'real_A',
                             'real_B', 'real_C', 'real_D', 'real_E', 'real_F', 'real_G', 'real_H', 'real_input']
        self.model_names = ['D', 'E', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'H7', 'H8']
        ...
        self.netH7 = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, 'head', opt.normG,
                                       not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias,
                                       opt.no_antialias_up, self.gpu_ids, opt)
        self.netH8 = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, 'head', opt.normG,
                                       not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias,
                                       opt.no_antialias_up, self.gpu_ids, opt)
    ...
    def forward(self):
        ...
        self.real_input = (self.real_A * self.label[0] + self.real_B * self.label[1] + self.real_C * self.label[2] +
                           ...
                           self.real_G * self.label[6] + self.real_H * self.label[7]) / label_sum
        ...
        self.fake_H = self.netH8(self.fake_all)
    ...
    def compute_D_loss(self):
        ...
        self.loss_D_fake = self.criterionGAN(pred_fake1, False) * self.label[0] \
                           ...
                           + self.criterionGAN(pred_fake8, False) * self.label[7]
        ...
        self.pred_real8 = self.netD(0,self.real_H)
        ...
        self.loss_D_real = self.criterionGAN(self.pred_real1, True) * self.label[0] \
                           ...
                           + self.criterionGAN(self.pred_real8, True) * self.label[7]
    ...
    def compute_G_loss(self):
        ...
        pred_fake8 = self.netD(0,self.fake_H)
        ...
        self.loss_G_GAN = self.criterionGAN(pred_fake1, True) * self.label[0] \
                          ...
                          + self.criterionGAN(pred_fake8, True) * self.label[7]
        ...
        self.loss_Ln = self.criterionL1(self.real_A, self.fake_A) * self.label[0] \
                       ...
                       + self.criterionL1(self.real_H, self.fake_H) * self.label[7]
        ...
        self.loss_VGG = self.criterionVGG(self.fake_A, self.real_A) * self.label[0] \
                        ...
                        + self.criterionVGG(self.real_H, self.fake_H) * self.label[7]
    ...
    def forward_test(self):
        ...
        if self.opt.test_choice == 1:
            ...
            self.real_input = (self.real_A * gt_label[0] + self.real_B * gt_label[1]
                               ...
                               + self.real_G * gt_label[6] + self.real_H * gt_label[7]) / sum(gt_label)
            self.fake_all = self.netE(self.real_input)
            self.predict_label = self.netD(1, self.real_input).view(self.opt.max_domain)
            ...
            self.fake_H = (self.fake_H + 1)/2
            self.real_H = (self.real_H + 1)/2
            ...
            if gt_label[7] == 1:
                mse = self.criterionL2(self.fake_H, self.real_H)
                psnr = 10 * log10(1 / mse.item())
                self.psnr_count[7] += psnr
            if predict_label.tolist() == gt_label:
                self.correct = self.correct + 1
            self.all_count = self.all_count + 1
            if self.all_count % 300 == 0:
                ...
                if gt_label[7] == 1:
                    print("PSNR_H: ", self.psnr_count[7] / self.all_count)
                self.all_count = 0
                self.correct = 0
                self.psnr_count = [0] * self.opt.max_domain
                self.acc_all += acc
